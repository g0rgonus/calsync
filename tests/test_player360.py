...
RUSH = Activity(
    ...
    league="TASL",
    age_group="U10",
    home_venue="Wolf Trap Park",
    aliases=("U10DA", "U10 DA", "Rush", "Rush Academy"),
    sport="soccer",
    emoji="⚽️",
    tz="America/New_York",
)
...
MILLIE = Child(id="millie", name="Millie", initial="M", birth_order=3)


@pytest.fixture(scope="module")
def result():
    return player360.parse_feed(
        FIXTURE.read_bytes(), RUSH, source_id="p360-james-rush"
    )
...
def test_league_match_yields_opponent_with_age_suffix_stripped(by_uid):
    # "U10DA TASL Match vs Beach FC U10" -> opponent "Beach FC"
    assert by_uid[MATCH].opponent == "Beach FC"
    assert by_uid[MATCH].detail is None


def test_practice_summary_strips_our_own_team_token(by_uid):
    # "U10DA Practice" -> "Practice". This is what makes the user-requested
    # "James ⚽️ Practice" fall out of the upstream-wins rule.
...
def test_venue_split_on_street_number(by_uid):
    ...
    assert venue.name == "Wolf Trap Park"
    assert venue.address == "1009 Wolf Trap Rd, Yorktown VA 23692"

    # The other venue uses a comma before the state; the split must not care.
    festival = by_uid[FESTIVAL].venue
    assert festival.name == "Randy Custis Memorial Park"
    assert festival.address == "7160 Rescue Ln, Exmore, VA 23350"
...
def test_home_away_derived_from_venue_not_the_vs_string(by_uid):
    # SUMMARY always reads "vs"; Wolf Trap Park is the home ground.
...
def test_description_dropped_when_it_duplicates_summary(by_uid):
    ...
    assert by_uid[MINICAMP].body == "Rush Academy Minicamp"  # differs, kept
...
def test_content_hash_ignores_sequence_and_last_modified():
    ...
    a = player360.parse_feed(original, RUSH, source_id="s")
    b = player360.parse_feed(churned, RUSH, source_id="s")
...
def test_content_hash_reacts_to_a_real_change():
    ...
    a = {e.uid: e.content_hash for e in player360.parse_feed(original, RUSH, source_id="s").events}
    b = {e.uid: e.content_hash for e in player360.parse_feed(moved, RUSH, source_id="s").events}
...
def test_empty_feed_is_not_evidence_of_cancellation():
    ...
    with pytest.raises(player360.FeedError, match="cancellation"):
        player360.parse_feed(empty, RUSH, source_id="s")


def test_garbage_feed_raises_rather_than_returning_nothing():
    with pytest.raises(player360.FeedError):
        player360.parse_feed("not a calendar at all", RUSH, source_id="s")
...
def test_titles(by_uid):
    assert _title(by_uid[MATCH], [JAMES]) == "James ⚽️ vs Beach FC"
    assert _title(by_uid[PRACTICE], [JAMES]) == "James ⚽️ Practice"
    assert _title(by_uid[FESTIVAL], [JAMES]) == "James ⚽️ Super 8v8 Festival Kickoff"


def test_multi_kid_titles_use_initials_in_birth_order(by_uid):
    event = by_uid[PRACTICE]
    assert _title(event, [JAMES, PATRICK]) == "P+J ⚽️ Practice"
    # Order of the input list must not matter; birth order decides.
    assert _title(event, [PATRICK, JAMES]) == "P+J ⚽️ Practice"
    assert _title(event, [JAMES, PATRICK, MILLIE]) == "Kids ⚽️ Practice"


def test_title_omits_venue(by_uid):
    assert "Wolf Trap" not in _title(by_uid[MATCH], [JAMES])
...
def test_single_kid_titles_fit_week_view(by_uid):
    ...
    for event in by_uid.values():
        head = _title(event, [JAMES])[:12]
        assert head.startswith("James ⚽️")
...
def test_away_only_marked_when_positively_known(by_uid):
    ...
    assert _title(event_unknown, [JAMES]) == "James ⚽️ vs Beach FC"

    event_away = type(event)(**{**event.__dict__, "home": False})
    assert _title(event_away, [JAMES]) == "James ⚽️ @ Beach FC"
