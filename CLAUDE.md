...
`Tests/.../Fixtures/games.ics` is generated by calsync's own pipeline — `Event`
through `render()` through `to_ics` — **input included**. Generating only the
output is not enough: the generator used to hand `to_ics` a `RenderedEvent` it
built itself with a zoned `starts_at`, so the fixture carried
`DTSTART;TZID=...` while Radicale has only ever served `DTSTART:...Z`, and the
parser was certified against a format that does not exist.
